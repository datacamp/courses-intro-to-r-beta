...
- Print the structure of `my_super_list` with the [`str()`](http://www.rdocumentation.org/packages/utils/functions/str) function.
...
You can also use the [`c()`](http://www.rdocumentation.org/packages/base/functions/c) function to add an element:

```
shining_list <- c(shining_list, my_opinion = "Love it!")
```

Notice that the [`c()`](http://www.rdocumentation.org/packages/base/functions/c) function does not automatically update the `shining_list` object with the additional element. You'll have to explicitly assign the result of the [`c()`](http://www.rdocumentation.org/packages/base/functions/c) call to `shining_list` to work with the extended list later on.
