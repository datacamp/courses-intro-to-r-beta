...
*** =instructions
Use `rbind()` to create `all_wars_matrix`, a new matrix with `star_wars_matrix` in the first three rows and `star_wars_matrix2` in the next three rows.
...
test_object("total_revenue_vector",
            ...
            incorrect_msg = "Have you correctly assigned the result of `colSums()` to `total_revenue_vector?`")
...
- Subset `visitors` so that you keep only the US visitors, and calculate the average number of this column. You have to use `mean()` here; this R function gives you the average of a numerical vector. Store the result in a variable `average_us_visitors`.
...
test_object("average_us_visitors", incorrect_msg = "It looks like `average_us_visitors` does not contain the average of the US visitors. Use `mean()` in combination with a subset of `visitors`.")
