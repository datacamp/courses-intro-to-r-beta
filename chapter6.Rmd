...
Alright, now let us do something useful with the [`order()`](http://www.rdocumentation.org/packages/base/functions/order) function! You would like to rearrange your data frame such that it starts with the smallest planet and ends with the largest one. A sort on the `diameter` column, as you will.
...
*** =hint
- `order(planets_df$diameter)` will give you the ordering of the variable diameter from smallest to largest. This is wat you should assign to `positions`.
