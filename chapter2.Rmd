...
*** =sct
```{r eval=FALSE}
test_object("poker_vector", incorrect_msg = "Don't change how `poker_vector` is defined.")
test_object("roulette_vector", incorrect_msg = "Make sure that you assign a vector with the correct values to `roulette_vector`.")
...
*** =instructions
Go ahead and assign the days of the week as names to `poker_vector` and `roulette_vector`. In case you are not sure, the days of the week are: Monday, Tuesday, Wednesday, Thursday and Friday. R is case sensitive!
...
test_object("poker_vector", incorrect_msg = sprintf(msg, "poker_vector"))
test_object("roulette_vector", incorrect_msg = sprintf(msg, "roulette_vector"))
test_object("days_vector", incorrect_msg = "Make sure that you assign the correct vector to `days_vector`. The names of the day should start with a capital letter!")
msg <- "Make sure that you assign `days_vector` to the names of `%s`."
test_object("poker_vector", eq_condition = "equal", incorrect_msg = sprintf(msg, "poker_vector"))
test_object("roulette_vector", eq_condition = "equal", incorrect_msg = sprintf(msg, "roulette_vector"))
...
*** =instructions
- Assign to the variable `total_daily` how much you won or lost on each day in total (poker and roulette combined). `total_daily` should be a vector of length 5.
...
success_msg("Good job! Head over to the next exercise. Just like selecting single elements using numerics extends naturally to selecting multiple elements, you can also use a vector of names.");
```
